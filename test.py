...
def scrape_article_details():
    ...
    for news_item in news_list:
        try:
            ...
            # Paraphrasing the title and content
            paraphrased_title = model.generate_content(f"Paraphrase the following title and return a single paraphrased title with no addintional information: {news_item['title']}").text
            paraphrased_content = model.generate_content(f"Paraphrase the following contentand return the paraphrased content in a markup format: {content}").text

            detailed_article = {
                'original_title': news_item['title'],
                'paraphrased_title': paraphrased_title,
                'category': category.text if category else 'No category',
